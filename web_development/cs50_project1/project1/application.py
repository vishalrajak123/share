...
def book(isbn):
    if request.method=="POST":
        ...
        comment=request.method.get("comment")
        row1=db.execute("SELECT id FROM books WHERE isbn=:isbn",{"isbn":isbn})
        bookId=row1.fetchone()
        bookId=bookId[0]
        row2=db.execute("SELECT * FROM reviews WHERE user_id=:user_id,book_id=:book_id",{"user_id":user_id,"book_id":book_id})
        if row2.rowcount==1:
            flash("you have already submited review")
            return redirect("/books/"+isbn)
        rating=int(rating)
        db.execute("INSERT INTO reviews (user_id,book_id,comment,rating) VALUES (:user_id,:book_id,:comment,:rating)",{"user_id":user_id,"book_id":book_id,"comment":comment,"rating":rating})
        db.commit()
